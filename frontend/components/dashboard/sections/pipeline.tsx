...
const CHAIN_META: Record<string, { label: string; token: string; consensus: string; color: string; type: string }> = {
  ETH: { label: "Ethereum", token: "ETH", consensus: "PoS", color: "bg-blue-500/10 text-blue-400 border-blue-500/20", type: "EVM" },
  BNB: { label: "BNB Chain", token: "BNB", consensus: "PoSA", color: "bg-yellow-500/10 text-yellow-400 border-yellow-500/20", type: "EVM" },
  ARB: { label: "Arbitrum", token: "ETH", consensus: "PoS", color: "bg-sky-500/10 text-sky-400 border-sky-500/20", type: "EVM" },
  OP: { label: "Optimism", token: "ETH", consensus: "PoS", color: "bg-red-500/10 text-red-400 border-red-500/20", type: "EVM" },
  BASE: { label: "Base", token: "ETH", consensus: "PoS", color: "bg-indigo-500/10 text-indigo-400 border-indigo-500/20", type: "EVM" },
  AVAX: { label: "Avalanche", token: "AVAX", consensus: "PoS", color: "bg-red-600/10 text-red-500 border-red-600/20", type: "EVM" },
  XLAYER: { label: "X Layer", token: "OKB", consensus: "PoA", color: "bg-slate-500/10 text-slate-400 border-slate-500/20", type: "EVM" },
  POLYGON: { label: "Polygon", token: "POL", consensus: "PoA", color: "bg-purple-500/10 text-purple-400 border-purple-500/20", type: "EVM" },
  BTC: { label: "Bitcoin", token: "BTC", consensus: "PoW", color: "bg-orange-500/10 text-orange-400 border-orange-500/20", type: "BTC" },
  SOLANA: { label: "Solana", token: "SOL", consensus: "PoH", color: "bg-green-500/10 text-green-400 border-green-500/20", type: "SOLANA" },
  TRON: { label: "Tron", token: "TRX", consensus: "DPoS", color: "bg-rose-500/10 text-rose-400 border-rose-500/20", type: "TRON" },
};

const CHAIN_ORDER = Object.keys(CHAIN_META) as Array<keyof typeof CHAIN_META>;
...
function formatStatusLabel(status: string, locale: "en" | "zn" | "vn") {
  switch (status) {
    case "loading":
      return locale === "en" ? "Checking" : locale === "zn" ? "检查中" : "Đang kiểm tra";
    case "online":
...
function buildPendingNode(chain: keyof typeof CHAIN_META): ChainStatus {
  return {
    chain,
    type: CHAIN_META[chain].type,
    status: "loading",
    rpc_env_name: null,
    error: undefined,
  };
      ...
      >
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-1.5">
            <span className={cn("h-2 w-2 rounded-full", statusClasses.dot, (node.status === "online" || node.status === "loading") && "animate-pulse")} />
        ...
        <tr className="border-b border-border bg-secondary/20">
          <td colSpan={8} className="px-6 py-4">
            <div className="grid grid-cols-2 gap-x-10 gap-y-2 text-xs sm:grid-cols-3 lg:grid-cols-5">
              ...
              {node.status === "loading" && !node.error ? (
                <div className="col-span-full mt-1">
                  <span className="text-sky-400/80">
                    {locale === "en" ? "Checking node status..." : locale === "zn" ? "正在检查节点状态..." : "Đang kiểm tra trạng thái node..."}
                  </span>
                </div>
              ) : null}
            </div>
...
export function PipelineSection() {
  ...
  const [nodes, setNodes] = useState<ChainStatus[]>(() => CHAIN_ORDER.map((chain) => buildPendingNode(chain)));
  ...
  const fetchStatus = useCallback(async () => {
    ...
    setSpinning(true);
    if (!lastUpdated) {
      setNodes(CHAIN_ORDER.map((chain) => buildPendingNode(chain)));
    }
    ...
    await Promise.allSettled(
      CHAIN_ORDER.map(async (chain) => {
        try {
          const response = await fetch(buildApiUrl(`/status/${chain}`), { cache: "no-store" });
          const payload = (await readApiPayload(response)) as { status?: ChainStatus; checked_at?: string; detail?: string } | null;
          if (!response.ok) {
            throw new Error((payload as { detail?: string } | null)?.detail ?? `HTTP ${response.status}`);
          }
          const nextStatus = payload?.status;
          if (!nextStatus) {
            throw new Error(locale === "en" ? "No node status returned" : locale === "zn" ? "节点状态缺失" : "Thiếu trạng thái node");
          }

          successfulRequests += 1;
          latestCheckedAt = payload?.checked_at ?? latestCheckedAt;
          if (requestId !== requestIdRef.current) return;

          setNodes((current) =>
            current.map((node) => (
              node.chain === chain
                ? { ...buildPendingNode(chain), ...nextStatus }
                : node
            )),
          );
        } catch (fetchError) {
    ...
    if (successfulRequests > 0) {
      ...
      setCheckedAt(latestCheckedAt);
    } else {
      setError(firstError ?? (locale === "en" ? "Failed to fetch node status" : locale === "zn" ? "获取节点状态失败" : "Tải trạng thái node thất bại"));
    }
  ...
  const loading = nodes.filter((node) => node.status === "loading").length;
  const configured = nodes.filter((node) => node.status === "online" || node.status === "offline").length;
  const unconfigured = nodes.filter((node) => node.status === "unconfigured").length;
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">
              {loading > 0 && !lastUpdated
                ? locale === "en"
                  ? `Checking ${loading} nodes...`
                  : locale === "zn"
                    ? `正在检查 ${loading} 个节点...`
                    : `Đang kiểm tra ${loading} node...`
                : error && online === 0
                  ? `${locale === "en" ? "Connection error" : locale === "zn" ? "连接错误" : "Lỗi kết nối"}: ${error}`
                  : loading > 0
                    ? locale === "en"
                      ? `${online} online • ${loading} checking`
                      : locale === "zn"
                        ? `${online} 个在线 • ${loading} 个检查中`
                        : `${online} trực tuyến • ${loading} đang kiểm tra`
                  : configured === 0
                    ? locale === "en"
                      ? "No nodes configured"
                      : locale === "zn"
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        ...
        <span className="flex items-center gap-1.5 rounded-full border border-sky-500/20 bg-sky-500/10 px-3 py-1 font-medium text-sky-400">
          <span className={cn("h-1.5 w-1.5 rounded-full bg-sky-400", loading > 0 && "animate-pulse")} />
          {loading} {locale === "en" ? "Checking" : locale === "zn" ? "检查中" : "Đang kiểm tra"}
        </span>
      ...
      <div className="overflow-hidden rounded-xl border border-border/80 bg-card shadow-[0_8px_22px_-18px_rgba(15,23,42,0.12)]">
        ...
        <div className="border-t border-border/80 px-4 py-2.5 text-xs text-muted-foreground">
          {locale === "en"
            ? `Click any row to expand full details${checkedAt ? ` | Checked at ${checkedAt}` : ""}`
            : locale === "zn"
              ? `点击任意一行展开完整详情${checkedAt ? ` | 检查时间 ${checkedAt}` : ""}`
              : `Nhấn vào bất kỳ hàng nào để mở rộng chi tiết${checkedAt ? ` | Kiểm tra lúc ${checkedAt}` : ""}`}
        </div>
      </div>

      {error && online === 0 && loading === 0 && (
